...
| FR-007 | Filter zurücksetzen | As a Fantasy-Football-Manager, I want to reset all active filters with one action, so that I can quickly return to the full player list. | Low | Removed — obsolete once position (pills, FR-003) and max rank (`rank:` search token, FR-004) stopped being separate dropdown/input filters; the two remaining criteria (Draft-Status, Suche) reset directly, making a dedicated action redundant. |
