...
const MATCHUP_RATINGS_PATH = new URL('../matchup-ratings.json', import.meta.url);
...
// `copy(JSON.stringify(advancedMetrics))`) into matchup-ratings.json.
// A leading `//` comment line in that file (documenting the exact command
// above) is tolerated and stripped before parsing.
async function loadMatchupRatings() {
  let raw;
  try {
    raw = await readFile(MATCHUP_RATINGS_PATH, 'utf8');
  } catch (err) {
    if (err.code === 'ENOENT') {
      console.log('No matchup-ratings.json found, continuing without matchup ratings.');
      return {};
    }
    throw err;
  }
  ...
  try {
    ...
    console.error('Could not parse matchup-ratings.json, continuing without matchup ratings:', err.message);
    return {};
  }
}
...
function mapPlayers(rawPlayers, opponents, matchupRatings) {
  return rawPlayers
    .map((p) => {
